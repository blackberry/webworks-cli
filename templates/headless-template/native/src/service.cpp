		...
		m_invokeManager(new InvokeManager(this))
{
	...
	m_notify->setBody("Headless service requires attention");

	bb::system::InvokeRequest request;
	request.setTarget("com.example.HeadlessUI");
...
void Service::handleInvoke(const bb::system::InvokeRequest & request) {
	if (request.action().compare("com.example.HeadlessService.RESET") == 0) {
		triggerNotification();
	}
}
